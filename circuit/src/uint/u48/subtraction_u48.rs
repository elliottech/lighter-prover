...
// Originally from: https://github.com/JumpCrypto/plonky2-crypto/blob/main/src/u32/gates/subtraction_u32.rs
...
pub struct U48SubtractionGate<F: RichField + Extendable<D>, const D: usize> {
    pub num_ops: usize,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize> U48SubtractionGate<F, D> {
    ...
    // We have limbs for the 48 bits of `output_result`.
...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for U48SubtractionGate<F, D> {
    fn id(&self) -> String {
        format!("{self:?}")
    }
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<F::Extension> {
        ...
        for i in 0..self.num_ops {
            ...
            let base = F::Extension::from_canonical_u64(1 << 48u64);
            ...
            // Range-check output_result to be at most 48 bits.
    ...
    fn eval_unfiltered_circuit(
        ...
        for i in 0..self.num_ops {
            ...
            let base = builder.constant_extension(F::Extension::from_canonical_u64(1 << 48u64));
...
pub struct U48SubtractionGenerator<F: RichField + Extendable<D>, const D: usize> {
    gate: U48SubtractionGate<F, D>,
    row: usize,
    i: usize,
    _phantom: PhantomData<F>,
}
...
impl<F: RichField + Extendable<D>, const D: usize> SimpleGenerator<F, D>
    for U48SubtractionGenerator<F, D>
{
    fn id(&self) -> String {
        "U48SubtractionGenerator".to_string()
    }
    ...
    fn run_once(
        ...
        let output_borrow = if result_initial_u64 > 1 << 48u64 {
            F::ONE
        } else {
            F::ZERO
        };
    ...
    fn deserialize(src: &mut Buffer, common_data: &CommonCircuitData<F, D>) -> IoResult<Self> {
        let gate = U48SubtractionGate::deserialize(src, common_data)?;
...
mod tests {
    ...
    fn test_gate_constraint() {
        ...
        const D: usize = 4;
        const NUM_U48_SUBTRACTION_OPS: usize = 3;

        fn get_wires(inputs_x: Vec<u64>, inputs_y: Vec<u64>, borrows: Vec<u64>) -> Vec<FF> {
            let mut v0 = Vec::new();
            let mut v1 = Vec::new();

            let limb_bits = U48SubtractionGate::<F, D>::limb_bits();
            let num_limbs = U48SubtractionGate::<F, D>::num_limbs();
            let limb_base = 1 << limb_bits;
            for c in 0..NUM_U48_SUBTRACTION_OPS {
        ...
        let mut rng = OsRng;
        let inputs_x = (0..NUM_U48_SUBTRACTION_OPS)
            .map(|_| rng.r#gen::<u64>() & 0x0000_FFFF_FFFF_FFFF)
            .collect();
        let inputs_y = (0..NUM_U48_SUBTRACTION_OPS)
            .map(|_| rng.r#gen::<u64>() & 0x0000_FFFF_FFFF_FFFF)
            .collect();
        let borrows = (0..NUM_U48_SUBTRACTION_OPS)
            .map(|_| (rng.r#gen::<u64>() & 0x0000_FFFF_FFFF_FFFF) % 2)
            .collect();

        let gate = U48SubtractionGate::<F, D> {
            num_ops: NUM_U48_SUBTRACTION_OPS,
            _phantom: PhantomData,
        };
