...
lazy_static! {
    static ref CUSTOM_GATE_SIZES_SET: HashSet<usize> = {
...
{
    #[track_caller]
    pub(crate) fn register_range_check_byte_decom(&mut self, val: Target, bit_size: usize) {
        assert!(
            bit_size <= 64,
    ...
    pub(crate) fn perform_registered_range_checks_byte_decom(&mut self) {
...
mod tests {
    ...
    macro_rules! impl_range_check_byte_decom_tests {
        ($bit_size:expr) => {
            paste! {
                #[test]
    ...
    macro_rules! impl_range_check_byte_decom_all_tests {
        ($($bit_size:expr),*) => {
            $(
                generate_low_degree_tests!($bit_size);
                impl_range_check_byte_decom_tests!($bit_size);
                generate_eval_fns_tests!($bit_size);
            )*
        };
    }

    impl_range_check_byte_decom_all_tests!(
