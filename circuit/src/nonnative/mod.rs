// Portions of this file are derived from plonky2-ecdsa
// Copyright (c) 2022 The Plonky2 Authors
// Licensed under the MIT License. See THIRD_PARTY_NOTICES for details.

// Modifications copyright (c) 2025 Elliot Technologies, Inc.
// This file has been modified from its original version.

use core::marker::PhantomData;

use anyhow::Result;
use num::{BigUint, Integer, One, Zero};
use plonky2::field::extension::Extendable;
use plonky2::field::types::{Field, PrimeField};
use plonky2::hash::hash_types::RichField;
use plonky2::iop::generator::{GeneratedValues, SimpleGenerator};
use plonky2::iop::target::{BoolTarget, Target};
use plonky2::iop::witness::{PartitionWitness, WitnessWrite};
use plonky2::plonk::circuit_data::CommonCircuitData;
use plonky2::util::serialization::{Buffer, IoResult, Read, Write};

use crate::bigint::biguint::{
    BigUintTarget, CircuitBuilderBiguint, GeneratedValuesBigUint, WitnessBigUint,
};
use crate::bigint::comparison::CircuitBuilderBiguintSubtractiveComparison;
use crate::bigint::div_rem::CircuitBuilderBiguintDivRem;
use crate::builder::Builder;
use crate::byte::split::CircuitBuilderByteSplit;
use crate::uint::u8::U8Target;
use crate::uint::u32::gadgets::arithmetic_u32::{CircuitBuilderU32, U32Target};
use crate::uint::u32::serialization::{ReadU32, WriteU32};
use crate::uint::u32::witness::GeneratedValuesU32;
use crate::utils::ceil_div_usize;
pub mod split_nonnative;

#[derive(Clone, Debug, Default)]
pub struct NonNativeTarget<FF: Field> {
    pub value: BigUintTarget,
    pub _phantom: PhantomData<FF>,
}

impl<FF: Field> From<BigUintTarget> for NonNativeTarget<FF> {
    fn from(value: BigUintTarget) -> Self {
        NonNativeTarget {
            value,
            _phantom: PhantomData,
        }
    }
}

pub trait CircuitBuilderNonNative<F: RichField + Extendable<D>, const D: usize> {
    fn num_nonnative_limbs<FF: Field>() -> usize {
        ceil_div_usize(FF::BITS, 32)
    }

    fn biguint_to_nonnative<FF: Field>(&mut self, x: &BigUintTarget) -> NonNativeTarget<FF>;

    fn nonnative_to_canonical_biguint<FF: Field>(
        &mut self,
        x: &NonNativeTarget<FF>,
    ) -> BigUintTarget;

    fn constant_nonnative<FF: PrimeField>(&mut self, x: FF) -> NonNativeTarget<FF>;

    fn zero_nonnative<FF: PrimeField>(&mut self) -> NonNativeTarget<FF>;

    // Assert that two NonNativeTarget's, both assumed to be in reduced form, are equal.
    fn connect_nonnative<FF: Field>(
        &mut self,
        lhs: &NonNativeTarget<FF>,
        rhs: &NonNativeTarget<FF>,
    );

    // Assert that two NonNativeTarget's, both assumed to be in reduced form, are equal.
    fn conditional_connect_nonnative<FF: Field>(
        &mut self,
        flag: BoolTarget,
        lhs: &NonNativeTarget<FF>,
        rhs: &NonNativeTarget<FF>,
    );

    fn add_virtual_nonnative_target<FF: Field>(&mut self) -> NonNativeTarget<FF>;

    fn add_virtual_nonnative_target_sized<FF: Field>(
        &mut self,
        num_limbs: usize,
    ) -> NonNativeTarget<FF>;

    fn add_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF>;

    fn mul_nonnative_by_bool<FF: Field>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: BoolTarget,
    ) -> NonNativeTarget<FF>;

    fn if_nonnative<FF: PrimeField>(
        &mut self,
        b: BoolTarget,
        x: &NonNativeTarget<FF>,
        y: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF>;

    fn add_many_nonnative<FF: PrimeField>(
        &mut self,
        to_add: &[NonNativeTarget<FF>],
    ) -> NonNativeTarget<FF>;

    // Subtract two `NonNativeTarget`s.
    fn sub_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF>;

    fn div_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF>;

    fn mul_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF>;

    fn mul_div_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
        d: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF>;

    fn mul_many_nonnative<FF: PrimeField>(
        &mut self,
        to_mul: &[NonNativeTarget<FF>],
    ) -> NonNativeTarget<FF>;

    fn neg_nonnative<FF: PrimeField>(&mut self, x: &NonNativeTarget<FF>) -> NonNativeTarget<FF>;

    fn inv_nonnative<FF: PrimeField>(
        &mut self,
        x: &NonNativeTarget<FF>,
        num_limbs: Option<usize>,
    ) -> NonNativeTarget<FF>;
    fn conditional_inv_nonnative<FF: PrimeField>(
        &mut self,
        cond: BoolTarget,
        x: &NonNativeTarget<FF>,
        num_limbs: Option<usize>,
    ) -> NonNativeTarget<FF>;

    /// Returns `x % |FF|` as a `NonNativeTarget`.
    fn reduce<FF: Field>(&mut self, x: &BigUintTarget) -> NonNativeTarget<FF>;

    fn reduce_nonnative<FF: Field>(&mut self, x: &NonNativeTarget<FF>) -> NonNativeTarget<FF>;

    fn bool_to_nonnative<FF: Field>(&mut self, b: &BoolTarget) -> NonNativeTarget<FF>;

    // Split a nonnative field element to bits.
    fn split_nonnative_to_bits<FF: Field>(&mut self, x: &NonNativeTarget<FF>) -> Vec<BoolTarget>;
    fn split_nonnative_to_bytes<FF: Field>(&mut self, x: &NonNativeTarget<FF>) -> Vec<U8Target>;

    fn nonnative_conditional_neg<FF: PrimeField>(
        &mut self,
        x: &NonNativeTarget<FF>,
        b: BoolTarget,
    ) -> NonNativeTarget<FF>;

    fn select_nonnative<FF: Field>(
        &mut self,
        flag: BoolTarget,
        lhs: &NonNativeTarget<FF>,
        rhs: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF>;
}

impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilderNonNative<F, D> for Builder<F, D> {
    fn num_nonnative_limbs<FF: Field>() -> usize {
        ceil_div_usize(FF::BITS, 32)
    }

    fn biguint_to_nonnative<FF: Field>(&mut self, x: &BigUintTarget) -> NonNativeTarget<FF> {
        NonNativeTarget {
            value: x.clone(),
            _phantom: PhantomData,
        }
    }

    fn nonnative_to_canonical_biguint<FF: Field>(
        &mut self,
        x: &NonNativeTarget<FF>,
    ) -> BigUintTarget {
        x.value.clone()
    }

    fn constant_nonnative<FF: PrimeField>(&mut self, x: FF) -> NonNativeTarget<FF> {
        let x_biguint = self.constant_biguint(&x.to_canonical_biguint());
        self.biguint_to_nonnative(&x_biguint)
    }

    fn zero_nonnative<FF: PrimeField>(&mut self) -> NonNativeTarget<FF> {
        self.constant_nonnative(FF::ZERO)
    }

    // Assert that two NonNativeTarget's, both assumed to be in reduced form, are equal.
    fn connect_nonnative<FF: Field>(
        &mut self,
        lhs: &NonNativeTarget<FF>,
        rhs: &NonNativeTarget<FF>,
    ) {
        self.connect_biguint(&lhs.value, &rhs.value);
    }

    // Assert that two NonNativeTarget's, both assumed to be in reduced form, are equal.
    fn conditional_connect_nonnative<FF: Field>(
        &mut self,
        flag: BoolTarget,
        lhs: &NonNativeTarget<FF>,
        rhs: &NonNativeTarget<FF>,
    ) {
        self.conditional_assert_eq_biguint(flag, &lhs.value, &rhs.value);
    }

    fn add_virtual_nonnative_target<FF: Field>(&mut self) -> NonNativeTarget<FF> {
        let num_limbs = Self::num_nonnative_limbs::<FF>();
        let value = self.add_virtual_biguint_target_safe(num_limbs);

        NonNativeTarget {
            value,
            _phantom: PhantomData,
        }
    }

    fn add_virtual_nonnative_target_sized<FF: Field>(
        &mut self,
        num_limbs: usize,
    ) -> NonNativeTarget<FF> {
        let value = self.add_virtual_biguint_target_safe(num_limbs);

        NonNativeTarget {
            value,
            _phantom: PhantomData,
        }
    }

    fn add_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF> {
        let sum = self.add_virtual_nonnative_target::<FF>();
        let overflow = self.add_virtual_bool_target_safe();

        self.add_simple_generator(NonNativeAdditionGenerator::<F, D, FF> {
            a: a.clone(),
            b: b.clone(),
            sum: sum.clone(),
            overflow,
            _phantom: PhantomData,
        });

        let sum_expected = self.add_biguint(&a.value, &b.value);

        let modulus = self.constant_biguint(&FF::order());
        let mod_times_overflow = self.mul_biguint_by_bool(&modulus, overflow);
        let sum_actual = self.add_biguint(&sum.value, &mod_times_overflow);
        self.connect_biguint(&sum_expected, &sum_actual);

        // We leave unreduced until necessary (e.g. when connecting values).
        sum
    }

    fn mul_nonnative_by_bool<FF: Field>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: BoolTarget,
    ) -> NonNativeTarget<FF> {
        NonNativeTarget {
            value: self.mul_biguint_by_bool(&a.value, b),
            _phantom: PhantomData,
        }
    }

    fn if_nonnative<FF: PrimeField>(
        &mut self,
        b: BoolTarget,
        x: &NonNativeTarget<FF>,
        y: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF> {
        let not_b = self.not(b);
        let maybe_x = self.mul_nonnative_by_bool(x, b);
        let maybe_y = self.mul_nonnative_by_bool(y, not_b);
        self.add_nonnative(&maybe_x, &maybe_y)
    }

    fn add_many_nonnative<FF: PrimeField>(
        &mut self,
        to_add: &[NonNativeTarget<FF>],
    ) -> NonNativeTarget<FF> {
        if to_add.len() == 1 {
            return to_add[0].clone();
        }

        let sum = self.add_virtual_nonnative_target::<FF>();
        let overflow = self.add_virtual_u32_target_safe();
        let summands = to_add.to_vec();

        self.add_simple_generator(NonNativeMultipleAddsGenerator::<F, D, FF> {
            summands: summands.clone(),
            sum: sum.clone(),
            overflow,
            _phantom: PhantomData,
        });

        let sum_expected = summands
            .iter()
            .fold(self.zero_biguint(), |a, b| self.add_biguint(&a, &b.value));

        let modulus = self.constant_biguint(&FF::order());
        let overflow_biguint = BigUintTarget {
            limbs: vec![overflow],
        };
        let mod_times_overflow = self.mul_biguint(&modulus, &overflow_biguint);
        let sum_actual = self.add_biguint(&sum.value, &mod_times_overflow);
        self.connect_biguint(&sum_expected, &sum_actual);

        // Range-check result.

        let cmp = self.is_lte_biguint(&sum.value, &modulus);
        let one = self.one();
        self.connect(cmp.target, one);

        sum
    }

    // Subtract two `NonNativeTarget`s.
    fn sub_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF> {
        let diff = self.add_virtual_nonnative_target::<FF>();
        let overflow = self.add_virtual_bool_target_safe();

        self.add_simple_generator(NonNativeSubtractionGenerator::<F, D, FF> {
            a: a.clone(),
            b: b.clone(),
            diff: diff.clone(),
            overflow,
            _phantom: PhantomData,
        });

        let diff_plus_b = self.add_biguint(&diff.value, &b.value);
        let modulus = self.constant_biguint(&FF::order());
        let mod_times_overflow = self.mul_biguint_by_bool(&modulus, overflow);
        let diff_plus_b_reduced = self.sub_biguint(&diff_plus_b, &mod_times_overflow);
        self.connect_biguint(&a.value, &diff_plus_b_reduced);

        diff
    }

    fn div_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF> {
        let div = self.add_virtual_nonnative_target::<FF>();
        let overflow = self.add_virtual_biguint_target_safe(a.value.num_limbs());
        let add_to_a = self.add_virtual_bool_target_safe();

        self.add_simple_generator(NonNativeDivisionGenerator::<F, D, FF> {
            a: a.clone(),
            b: b.clone(),
            div: div.clone(),
            overflow: overflow.clone(),
            add_to_a,
            _phantom: PhantomData,
        });

        let b_times_div = self.mul_biguint(&b.value, &div.value);

        let add_rem_times_modulus_to = self.select_biguint(add_to_a, &a.value, &b_times_div);
        let lhs = self.select_biguint(add_to_a, &b_times_div, &a.value);

        let modulus = self.constant_biguint(&FF::order());
        let rem_times_modulus = self.mul_biguint(&overflow, &modulus);

        let rhs = self.add_biguint(&add_rem_times_modulus_to, &rem_times_modulus);

        self.connect_biguint(&lhs, &rhs);

        div
    }

    fn mul_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF> {
        let prod = self.add_virtual_nonnative_target::<FF>();
        let modulus = self.constant_biguint(&FF::order());
        let overflow = self.add_virtual_biguint_target_safe(
            a.value.num_limbs() + b.value.num_limbs() - modulus.num_limbs(),
        );

        self.add_simple_generator(NonNativeMultiplicationGenerator::<F, D, FF> {
            a: a.clone(),
            b: b.clone(),
            prod: prod.clone(),
            overflow: overflow.clone(),
            _phantom: PhantomData,
        });

        let prod_expected = self.mul_biguint(&a.value, &b.value);

        let mod_times_overflow = self.mul_biguint(&modulus, &overflow);
        let prod_actual = self.add_biguint(&prod.value, &mod_times_overflow);
        self.connect_biguint(&prod_expected, &prod_actual);

        prod
    }

    fn mul_div_nonnative<FF: PrimeField>(
        &mut self,
        a: &NonNativeTarget<FF>,
        b: &NonNativeTarget<FF>,
        d: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF> {
        let modulus = self.constant_biguint(&FF::order());
        let num_limbs = Self::num_nonnative_limbs::<FF>();

        let result = self.add_virtual_nonnative_target::<FF>();
        let overflow = self.add_virtual_biguint_target_safe(num_limbs);
        let add_to_lhs = self.add_virtual_bool_target_safe();

        self.add_simple_generator(NonNativeMulDivGenerator::<F, D, FF> {
            a: a.clone(),
            b: b.clone(),
            d: d.clone(),
            add_to_lhs,
            result: result.clone(),
            overflow: overflow.clone(),
            _phantom: PhantomData,
        });

        let lhs = self.mul_biguint(&a.value, &b.value);
        let rhs = self.mul_biguint(&d.value, &result.value);

        // verify that lhs == rhs (mod modulus) by providing quotient `overflow`
        // such that lhs + overflow * modulus = rhs
        // If both a and b are reduced, then lhs < modulus^2.
        // rhs is also < modulus^2, since result < modulus (otherwise we could reduce it)
        // so overflow < modulus.

        let mod_times_overflow = self.mul_biguint(&modulus, &overflow);

        let lhs_actual = self.add_biguint(&lhs, &mod_times_overflow);
        let rhs_actual = self.add_biguint(&rhs, &mod_times_overflow);

        let x = self.select_biguint(add_to_lhs, &lhs_actual, &lhs); // lhs
        let y = self.select_biguint(add_to_lhs, &rhs, &rhs_actual); // rhs

        self.connect_biguint(&x, &y);

        result
    }

    fn mul_many_nonnative<FF: PrimeField>(
        &mut self,
        to_mul: &[NonNativeTarget<FF>],
    ) -> NonNativeTarget<FF> {
        if to_mul.len() == 1 {
            return to_mul[0].clone();
        }

        let mut accumulator = self.mul_nonnative(&to_mul[0], &to_mul[1]);
        for t in to_mul.iter().skip(2) {
            accumulator = self.mul_nonnative(&accumulator, t);
        }
        accumulator
    }

    fn neg_nonnative<FF: PrimeField>(&mut self, x: &NonNativeTarget<FF>) -> NonNativeTarget<FF> {
        let zero_target = self.constant_biguint(&BigUint::zero());
        let zero_ff = self.biguint_to_nonnative(&zero_target);

        self.sub_nonnative(&zero_ff, x)
    }

    fn inv_nonnative<FF: PrimeField>(
        &mut self,
        x: &NonNativeTarget<FF>,
        num_limbs: Option<usize>,
    ) -> NonNativeTarget<FF> {
        let num_limbs = num_limbs.unwrap_or_else(|| x.value.num_limbs());
        let inv_biguint = self.add_virtual_biguint_target_safe(num_limbs);
        let div = self.add_virtual_biguint_target_safe(num_limbs);

        // k * |FF| + 1
        self.add_simple_generator(NonNativeInverseGenerator::<F, D, FF> {
            x: x.clone(),
            inv: inv_biguint.clone(),
            div: div.clone(),
            _phantom: PhantomData,
        });

        let product = self.mul_biguint(&x.value, &inv_biguint);

        let modulus = self.constant_biguint(&FF::order());
        let mod_times_div = self.mul_biguint(&modulus, &div);
        let one = self.constant_biguint(&BigUint::one());
        let expected_product = self.add_biguint(&mod_times_div, &one);
        self.connect_biguint(&product, &expected_product);

        NonNativeTarget::<FF> {
            value: inv_biguint,
            _phantom: PhantomData,
        }
    }

    fn conditional_inv_nonnative<FF: PrimeField>(
        &mut self,
        cond: BoolTarget,
        x: &NonNativeTarget<FF>,
        num_limbs: Option<usize>,
    ) -> NonNativeTarget<FF> {
        let num_limbs = num_limbs.unwrap_or_else(|| x.value.num_limbs());
        let inv_biguint = self.add_virtual_biguint_target_safe(num_limbs);
        let div = self.add_virtual_biguint_target_safe(num_limbs);

        self.add_simple_generator(NonNativeInverseGenerator::<F, D, FF> {
            x: x.clone(),
            inv: inv_biguint.clone(),
            div: div.clone(),
            _phantom: PhantomData,
        });

        let product = self.mul_biguint(&x.value, &inv_biguint);

        let modulus = self.constant_biguint(&FF::order());
        let mod_times_div = self.mul_biguint(&modulus, &div);
        let one = self.constant_biguint(&BigUint::one());
        let expected_product = self.add_biguint(&mod_times_div, &one);
        self.conditional_assert_eq_biguint(cond, &product, &expected_product);

        NonNativeTarget::<FF> {
            value: inv_biguint,
            _phantom: PhantomData,
        }
    }

    /// Returns `x % |FF|` as a `NonNativeTarget`.
    fn reduce<FF: Field>(&mut self, x: &BigUintTarget) -> NonNativeTarget<FF> {
        let modulus = FF::order();
        let order_target = self.constant_biguint(&modulus);
        let value = self.rem_biguint(x, &order_target);

        NonNativeTarget {
            value,
            _phantom: PhantomData,
        }
    }

    fn reduce_nonnative<FF: Field>(&mut self, x: &NonNativeTarget<FF>) -> NonNativeTarget<FF> {
        let x_biguint = self.nonnative_to_canonical_biguint(x);
        self.reduce(&x_biguint)
    }

    fn bool_to_nonnative<FF: Field>(&mut self, b: &BoolTarget) -> NonNativeTarget<FF> {
        let limbs = vec![U32Target(b.target)];
        let value = BigUintTarget { limbs };

        NonNativeTarget {
            value,
            _phantom: PhantomData,
        }
    }

    // Split a nonnative field element to bits.
    fn split_nonnative_to_bits<FF: Field>(&mut self, x: &NonNativeTarget<FF>) -> Vec<BoolTarget> {
        let num_limbs = x.value.num_limbs();
        let mut result = Vec::with_capacity(num_limbs * 32);

        for i in 0..num_limbs {
            let limb = x.value.get_limb(i);
            let mut bits: Vec<_> = self.split_le(limb.0, 32).to_vec();

            result.append(&mut bits);
        }

        result
    }

    fn split_nonnative_to_bytes<FF: Field>(&mut self, x: &NonNativeTarget<FF>) -> Vec<U8Target> {
        let num_limbs = x.value.num_limbs();
        let mut result = Vec::with_capacity(num_limbs * 4);

        for i in 0..num_limbs {
            let limb = x.value.get_limb(i);
            let mut bytes: Vec<_> = self.split_bytes(limb.0, 4).to_vec();

            result.append(&mut bytes);
        }

        result
    }

    fn nonnative_conditional_neg<FF: PrimeField>(
        &mut self,
        x: &NonNativeTarget<FF>,
        b: BoolTarget,
    ) -> NonNativeTarget<FF> {
        let not_b = self.not(b);
        let neg = self.neg_nonnative(x);
        let x_if_true = self.mul_nonnative_by_bool(&neg, b);
        let x_if_false = self.mul_nonnative_by_bool(x, not_b);

        self.add_nonnative(&x_if_true, &x_if_false)
    }

    fn select_nonnative<FF: Field>(
        &mut self,
        flag: BoolTarget,
        lhs: &NonNativeTarget<FF>,
        rhs: &NonNativeTarget<FF>,
    ) -> NonNativeTarget<FF> {
        NonNativeTarget::<FF> {
            value: self.select_biguint(flag, &lhs.value, &rhs.value),
            _phantom: PhantomData,
        }
    }
}

#[derive(Debug, Default)]
pub struct NonNativeAdditionGenerator<F: RichField + Extendable<D>, const D: usize, FF: PrimeField>
{
    a: NonNativeTarget<FF>,
    b: NonNativeTarget<FF>,
    sum: NonNativeTarget<FF>,
    overflow: BoolTarget,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> SimpleGenerator<F, D>
    for NonNativeAdditionGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.a
            .value
            .limbs
            .iter()
            .cloned()
            .chain(self.b.value.limbs.clone())
            .map(|l| l.0)
            .collect()
    }

    fn run_once(
        &self,
        witness: &PartitionWitness<F>,
        out_buffer: &mut GeneratedValues<F>,
    ) -> Result<()> {
        let a = FF::from_noncanonical_biguint(witness.get_biguint_target(self.a.value.clone()));
        let b = FF::from_noncanonical_biguint(witness.get_biguint_target(self.b.value.clone()));
        let a_biguint = a.to_canonical_biguint();
        let b_biguint = b.to_canonical_biguint();
        let sum_biguint = a_biguint + b_biguint;
        let modulus = FF::order();
        let (overflow, sum_reduced) = if sum_biguint > modulus {
            (true, sum_biguint - modulus)
        } else {
            (false, sum_biguint)
        };

        out_buffer.set_biguint_target(&self.sum.value, &sum_reduced)?;
        out_buffer.set_bool_target(self.overflow, overflow)?;

        Ok(())
    }

    fn id(&self) -> String {
        format!("NonNativeAdditionGenerator_{:?}", FF::order()).to_string()
    }

    fn serialize(&self, dst: &mut Vec<u8>, _common_data: &CommonCircuitData<F, D>) -> IoResult<()> {
        dst.write_target_vec(
            &self
                .a
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .b
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .sum
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;

        dst.write_target_bool(self.overflow)
    }

    fn deserialize(src: &mut Buffer, _common_data: &CommonCircuitData<F, D>) -> IoResult<Self>
    where
        Self: Sized,
    {
        let a = src.read_target_vec()?;
        let b = src.read_target_vec()?;
        let sum = src.read_target_vec()?;

        let overflow = src.read_target_bool()?;

        Ok(Self {
            a: NonNativeTarget::from(BigUintTarget::from(
                a.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            b: NonNativeTarget::from(BigUintTarget::from(
                b.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            sum: NonNativeTarget::from(BigUintTarget::from(
                sum.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            )),
            overflow,
            _phantom: PhantomData,
        })
    }
}

#[derive(Debug, Default)]
pub struct NonNativeMultipleAddsGenerator<
    F: RichField + Extendable<D>,
    const D: usize,
    FF: PrimeField,
> {
    summands: Vec<NonNativeTarget<FF>>,
    sum: NonNativeTarget<FF>,
    overflow: U32Target,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> SimpleGenerator<F, D>
    for NonNativeMultipleAddsGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.summands
            .iter()
            .flat_map(|summand| summand.value.limbs.iter().map(|limb| limb.0))
            .collect()
    }

    fn run_once(
        &self,
        witness: &PartitionWitness<F>,
        out_buffer: &mut GeneratedValues<F>,
    ) -> Result<()> {
        let summands: Vec<_> = self
            .summands
            .iter()
            .map(|summand| {
                FF::from_noncanonical_biguint(witness.get_biguint_target(summand.value.clone()))
            })
            .collect();
        let summand_biguints: Vec<_> = summands
            .iter()
            .map(|summand| summand.to_canonical_biguint())
            .collect();

        let sum_biguint = summand_biguints
            .iter()
            .fold(BigUint::zero(), |a, b| a + b.clone());

        let modulus = FF::order();
        let (overflow_biguint, sum_reduced) = sum_biguint.div_rem(&modulus);
        let overflow = overflow_biguint.to_u64_digits()[0] as u32;

        out_buffer.set_biguint_target(&self.sum.value, &sum_reduced)?;
        out_buffer.set_u32_target(self.overflow, overflow)?;

        Ok(())
    }

    fn id(&self) -> String {
        format!("NonNativeMultipleAddsGenerator_{:?}", FF::order()).to_string()
    }

    fn serialize(
        &self,
        dst: &mut Vec<u8>,
        _: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<()> {
        dst.write_usize(self.summands.len())?;
        self.summands.iter().try_for_each(|summand| {
            dst.write_target_vec(
                &summand
                    .value
                    .limbs
                    .iter()
                    .map(|x| x.0)
                    .collect::<Vec<Target>>(),
            )
        })?;
        dst.write_target_vec(
            &self
                .sum
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_u32(self.overflow)?;

        Ok(())
    }

    fn deserialize(
        src: &mut plonky2::util::serialization::Buffer,
        _: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<Self>
    where
        Self: Sized,
    {
        let num_summands = src.read_usize()?;
        let mut summands = vec![];
        (0..num_summands).try_for_each(|_| {
            let summand = src.read_target_vec()?;
            summands.push(NonNativeTarget::from(BigUintTarget::from(
                summand
                    .iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            )));

            Ok(())
        })?;
        let sum = src.read_target_vec()?;
        let overflow = src.read_target_u32()?;

        Ok(Self {
            summands,
            sum: NonNativeTarget::from(BigUintTarget::from(
                sum.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            )),
            overflow,
            _phantom: PhantomData,
        })
    }
}

#[derive(Debug, Default)]
pub struct NonNativeSubtractionGenerator<F: RichField + Extendable<D>, const D: usize, FF: Field> {
    a: NonNativeTarget<FF>,
    b: NonNativeTarget<FF>,
    diff: NonNativeTarget<FF>,
    overflow: BoolTarget,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> SimpleGenerator<F, D>
    for NonNativeSubtractionGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.a
            .value
            .limbs
            .iter()
            .cloned()
            .chain(self.b.value.limbs.clone())
            .map(|l| l.0)
            .collect()
    }

    fn run_once(
        &self,
        witness: &PartitionWitness<F>,
        out_buffer: &mut GeneratedValues<F>,
    ) -> Result<()> {
        let a = FF::from_noncanonical_biguint(witness.get_biguint_target(self.a.value.clone()));
        let b = FF::from_noncanonical_biguint(witness.get_biguint_target(self.b.value.clone()));
        let a_biguint = a.to_canonical_biguint();
        let b_biguint = b.to_canonical_biguint();

        let modulus = FF::order();
        let (diff_biguint, overflow) = if a_biguint >= b_biguint {
            (a_biguint - b_biguint, false)
        } else {
            (modulus + a_biguint - b_biguint, true)
        };

        out_buffer.set_biguint_target(&self.diff.value, &diff_biguint)?;
        out_buffer.set_bool_target(self.overflow, overflow)?;

        Ok(())
    }

    fn id(&self) -> String {
        format!("NonNativeSubtractionGenerator_{:?}", FF::order()).to_string()
    }

    fn serialize(
        &self,
        dst: &mut Vec<u8>,
        _: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<()> {
        dst.write_target_vec(
            &self
                .a
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .b
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .diff
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_bool(self.overflow)?;

        Ok(())
    }

    fn deserialize(
        src: &mut plonky2::util::serialization::Buffer,
        _: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<Self>
    where
        Self: Sized,
    {
        let a = src.read_target_vec()?;
        let b = src.read_target_vec()?;
        let diff = src.read_target_vec()?;
        let overflow = src.read_target_bool()?;

        Ok(Self {
            a: NonNativeTarget::from(BigUintTarget::from(
                a.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            b: NonNativeTarget::from(BigUintTarget::from(
                b.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            diff: NonNativeTarget::from(BigUintTarget::from(
                diff.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            )),
            overflow,
            _phantom: PhantomData,
        })
    }
}

#[derive(Debug, Default)]
pub struct NonNativeMultiplicationGenerator<F: RichField + Extendable<D>, const D: usize, FF: Field>
{
    a: NonNativeTarget<FF>,
    b: NonNativeTarget<FF>,
    prod: NonNativeTarget<FF>,
    overflow: BigUintTarget,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> SimpleGenerator<F, D>
    for NonNativeMultiplicationGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.a
            .value
            .limbs
            .iter()
            .cloned()
            .chain(self.b.value.limbs.clone())
            .map(|l| l.0)
            .collect()
    }

    fn run_once(
        &self,
        witness: &PartitionWitness<F>,
        out_buffer: &mut GeneratedValues<F>,
    ) -> Result<()> {
        let a = FF::from_noncanonical_biguint(witness.get_biguint_target(self.a.value.clone()));
        let b = FF::from_noncanonical_biguint(witness.get_biguint_target(self.b.value.clone()));
        let a_biguint = a.to_canonical_biguint();
        let b_biguint = b.to_canonical_biguint();

        let prod_biguint = a_biguint * b_biguint;

        let modulus = FF::order();
        let (overflow_biguint, prod_reduced) = prod_biguint.div_rem(&modulus);

        out_buffer.set_biguint_target(&self.prod.value, &prod_reduced)?;
        out_buffer.set_biguint_target(&self.overflow, &overflow_biguint)?;

        Ok(())
    }

    fn id(&self) -> String {
        format!("NonNativeMultiplicationGenerator_{:?}", FF::order()).to_string()
    }

    fn serialize(
        &self,
        dst: &mut Vec<u8>,
        _common_data: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<()> {
        dst.write_target_vec(
            &self
                .a
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .b
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .prod
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .overflow
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;

        Ok(())
    }

    fn deserialize(
        src: &mut plonky2::util::serialization::Buffer,
        _common_data: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<Self>
    where
        Self: Sized,
    {
        let a = src.read_target_vec()?;
        let b = src.read_target_vec()?;
        let prod = src.read_target_vec()?;
        let overflow = src.read_target_vec()?;

        Ok(Self {
            a: NonNativeTarget::from(BigUintTarget::from(
                a.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            b: NonNativeTarget::from(BigUintTarget::from(
                b.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            prod: NonNativeTarget::from(BigUintTarget::from(
                prod.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            )),
            overflow: BigUintTarget::from(
                overflow
                    .iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            ),
            _phantom: PhantomData,
        })
    }
}

#[derive(Debug, Default)]
pub struct NonNativeMulDivGenerator<F: RichField + Extendable<D>, const D: usize, FF: Field> {
    a: NonNativeTarget<FF>,
    b: NonNativeTarget<FF>,
    d: NonNativeTarget<FF>,
    add_to_lhs: BoolTarget,
    result: NonNativeTarget<FF>,
    overflow: BigUintTarget,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> SimpleGenerator<F, D>
    for NonNativeMulDivGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.a
            .value
            .limbs
            .iter()
            .cloned()
            .chain(self.b.value.limbs.clone())
            .chain(self.d.value.limbs.clone())
            .map(|l| l.0)
            .collect()
    }

    fn run_once(
        &self,
        witness: &PartitionWitness<F>,
        out_buffer: &mut GeneratedValues<F>,
    ) -> Result<()> {
        let a = FF::from_noncanonical_biguint(witness.get_biguint_target(self.a.value.clone()));
        let b = FF::from_noncanonical_biguint(witness.get_biguint_target(self.b.value.clone()));
        let d = FF::from_noncanonical_biguint(witness.get_biguint_target(self.d.value.clone()));
        let a_biguint = a.to_canonical_biguint();
        let b_biguint = b.to_canonical_biguint();
        let d_biguint = d.to_canonical_biguint();

        let d_inv = FF::from_noncanonical_biguint(d_biguint.clone()).try_inverse();
        if d_inv.is_none() {
            return Err(anyhow::anyhow!(
                "Division by zero in NonNativeMulDivGenerator"
            ));
        }

        let d_inv_biguint = d_inv.unwrap().to_canonical_biguint();
        let prod_biguint = a_biguint.clone() * b_biguint.clone() * d_inv_biguint;

        let modulus = FF::order();
        let (_, prod_reduced) = prod_biguint.div_rem(&modulus);

        out_buffer.set_biguint_target(&self.result.value, &prod_reduced)?;

        let lhs = a_biguint * b_biguint;
        let rhs = d_biguint * prod_reduced;

        if lhs > rhs {
            let diff = lhs - rhs;
            let (overflow_biguint, _) = diff.div_rem(&modulus);
            out_buffer.set_biguint_target(&self.overflow, &overflow_biguint)?;
            out_buffer.set_bool_target(self.add_to_lhs, false)?;
        } else {
            let diff = rhs - lhs;
            let (overflow_biguint, _) = diff.div_rem(&modulus);
            out_buffer.set_biguint_target(&self.overflow, &overflow_biguint)?;
            out_buffer.set_bool_target(self.add_to_lhs, true)?;
        }

        Ok(())
    }

    fn id(&self) -> String {
        format!("NonNativeMulDivGenerator_{:?}", FF::order()).to_string()
    }

    fn serialize(
        &self,
        dst: &mut Vec<u8>,
        _common_data: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<()> {
        dst.write_target_vec(
            &self
                .a
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .b
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .d
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_bool(self.add_to_lhs)?;
        dst.write_target_vec(
            &self
                .result
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .overflow
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;

        Ok(())
    }

    fn deserialize(
        src: &mut plonky2::util::serialization::Buffer,
        _common_data: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<Self>
    where
        Self: Sized,
    {
        let a = src.read_target_vec()?;
        let b = src.read_target_vec()?;
        let d = src.read_target_vec()?;
        let add_to_lhs = src.read_target_bool()?;
        let result = src.read_target_vec()?;
        let overflow = src.read_target_vec()?;

        Ok(Self {
            a: NonNativeTarget::from(BigUintTarget::from(
                a.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            b: NonNativeTarget::from(BigUintTarget::from(
                b.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            d: NonNativeTarget::from(BigUintTarget::from(
                d.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            add_to_lhs,
            result: NonNativeTarget::from(BigUintTarget::from(
                result
                    .iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            )),
            overflow: BigUintTarget::from(
                overflow
                    .iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            ),
            _phantom: PhantomData,
        })
    }
}

#[derive(Debug, Default)]
pub struct NonNativeInverseGenerator<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> {
    x: NonNativeTarget<FF>,
    inv: BigUintTarget,
    div: BigUintTarget,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> SimpleGenerator<F, D>
    for NonNativeInverseGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.x.value.limbs.iter().map(|&l| l.0).collect()
    }

    fn run_once(
        &self,
        witness: &PartitionWitness<F>,
        out_buffer: &mut GeneratedValues<F>,
    ) -> Result<()> {
        let x = FF::from_noncanonical_biguint(witness.get_biguint_target(self.x.value.clone()));
        let inv = x.try_inverse();
        if inv.is_none() {
            out_buffer.set_biguint_target(&self.div, &BigUint::ZERO)?;
            out_buffer.set_biguint_target(&self.inv, &BigUint::ZERO)?;

            return Ok(());
        }
        let inv = inv.unwrap();

        let x_biguint = x.to_canonical_biguint();
        let inv_biguint = inv.to_canonical_biguint();
        let prod = x_biguint * &inv_biguint;
        let modulus = FF::order();
        let (div, _rem) = prod.div_rem(&modulus);

        out_buffer.set_biguint_target(&self.div, &div)?;
        out_buffer.set_biguint_target(&self.inv, &inv_biguint)?;

        Ok(())
    }

    fn id(&self) -> String {
        format!("NonNativeInverseGenerator_{:?}", FF::order()).to_string()
    }

    fn serialize(
        &self,
        dst: &mut Vec<u8>,
        _: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<()> {
        dst.write_target_vec(
            &self
                .x
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(&self.inv.limbs.iter().map(|x| x.0).collect::<Vec<Target>>())?;
        dst.write_target_vec(&self.div.limbs.iter().map(|x| x.0).collect::<Vec<Target>>())?;

        Ok(())
    }

    fn deserialize(
        src: &mut plonky2::util::serialization::Buffer,
        _: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<Self>
    where
        Self: Sized,
    {
        let x = src.read_target_vec()?;
        let inv = src.read_target_vec()?;
        let div = src.read_target_vec()?;

        Ok(Self {
            x: NonNativeTarget::from(BigUintTarget::from(
                x.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            inv: BigUintTarget::from(
                inv.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            ),
            div: BigUintTarget::from(
                div.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            ),
            _phantom: PhantomData,
        })
    }
}

/// a * (b_inv) = div + overflow * |FF|
#[derive(Debug, Default)]
pub struct NonNativeDivisionGenerator<F: RichField + Extendable<D>, const D: usize, FF: Field> {
    a: NonNativeTarget<FF>,
    b: NonNativeTarget<FF>,
    div: NonNativeTarget<FF>,
    overflow: BigUintTarget,
    add_to_a: BoolTarget,
    _phantom: PhantomData<F>,
}

impl<F: RichField + Extendable<D>, const D: usize, FF: PrimeField> SimpleGenerator<F, D>
    for NonNativeDivisionGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.a
            .value
            .limbs
            .iter()
            .cloned()
            .chain(self.b.value.limbs.clone())
            .map(|l| l.0)
            .collect()
    }

    fn run_once(
        &self,
        witness: &PartitionWitness<F>,
        out_buffer: &mut GeneratedValues<F>,
    ) -> Result<()> {
        let b = witness.get_biguint_target(self.b.value.clone());
        let b_inv = FF::from_noncanonical_biguint(b.clone()).try_inverse();
        if b_inv.is_none() {
            out_buffer.set_biguint_target(&self.div.value, &BigUint::ZERO)?;
            out_buffer.set_biguint_target(&self.overflow, &BigUint::ZERO)?;
            return Ok(());
        }
        let b_inv = b_inv.unwrap();

        let a = witness.get_biguint_target(self.a.value.clone());

        let order = FF::order();
        let (_, div) = (&a * b_inv.to_canonical_biguint()).div_rem(&order);

        let b_times_div = &b * &div;
        let (overflow, should_be_zero) = if b_times_div <= a {
            out_buffer.set_bool_target(self.add_to_a, false)?;
            (&a - b_times_div).div_rem(&order)
        } else {
            out_buffer.set_bool_target(self.add_to_a, true)?;
            (b_times_div - &a).div_rem(&order)
        };
        assert_eq!(should_be_zero, BigUint::ZERO);

        out_buffer.set_biguint_target(&self.div.value, &div)?;
        out_buffer.set_biguint_target(&self.overflow, &overflow)?;

        Ok(())
    }

    fn id(&self) -> String {
        format!("NonNativeDivisionGenerator_{:?}", FF::order()).to_string()
    }

    fn serialize(
        &self,
        dst: &mut Vec<u8>,
        _common_data: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<()> {
        dst.write_target_vec(
            &self
                .a
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .b
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .div
                .value
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_vec(
            &self
                .overflow
                .limbs
                .iter()
                .map(|x| x.0)
                .collect::<Vec<Target>>(),
        )?;
        dst.write_target_bool(self.add_to_a)?;

        Ok(())
    }

    fn deserialize(
        src: &mut plonky2::util::serialization::Buffer,
        _common_data: &plonky2::plonk::circuit_data::CommonCircuitData<F, D>,
    ) -> plonky2::util::serialization::IoResult<Self>
    where
        Self: Sized,
    {
        let a = src.read_target_vec()?;
        let b = src.read_target_vec()?;
        let div = src.read_target_vec()?;
        let rem = src.read_target_vec()?;
        let add_to_a = src.read_target_bool()?;

        Ok(Self {
            a: NonNativeTarget::from(BigUintTarget::from(
                a.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            b: NonNativeTarget::from(BigUintTarget::from(
                b.iter().map(|&x| U32Target(x)).collect::<Vec<U32Target>>(),
            )),
            div: NonNativeTarget::from(BigUintTarget::from(
                div.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            )),
            overflow: BigUintTarget::from(
                rem.iter()
                    .map(|&x| U32Target(x))
                    .collect::<Vec<U32Target>>(),
            ),
            add_to_a,
            _phantom: PhantomData,
        })
    }
}

#[cfg(test)]
mod tests {
    use anyhow::Result;
    use plonky2::field::types::Sample;
    use plonky2::iop::witness::PartialWitness;

    use super::*;
    use crate::blob::bls12_381_scalar_field::{BLS12_381_SCALAR_LIMBS, BLS12381Scalar};
    use crate::types::config::{C, CIRCUIT_CONFIG};

    #[test]
    fn nonnative_div() -> Result<()> {
        // let _ = env_logger::try_init_from_env(
        //     env_logger::Env::default().filter_or(env_logger::DEFAULT_FILTER_ENV, "debug"),
        // );

        let mut builder = Builder::new(CIRCUIT_CONFIG);

        let arr: [BLS12381Scalar; 50 * 2] = BLS12381Scalar::rand_array();
        for chunk in arr.chunks(2) {
            let a = builder.constant_nonnative::<BLS12381Scalar>(chunk[0]);
            let b = builder.constant_nonnative::<BLS12381Scalar>(chunk[1]);

            let b_inv = builder.inv_nonnative(&b, Some(BLS12_381_SCALAR_LIMBS));
            let c_1 = builder.mul_nonnative(&a, &b_inv);

            let c_2 = builder.div_nonnative(&a, &b);

            builder.connect_nonnative(&c_1, &c_2);
        }

        let data = builder.build::<C>();
        let proof = data.prove(PartialWitness::new()).unwrap();
        data.verify(proof)
    }

    #[test]
    fn test_mul_div_nonnative() -> Result<()> {
        let mut builder = Builder::new(CIRCUIT_CONFIG);

        let arr: [BLS12381Scalar; 1000 * 3] = BLS12381Scalar::rand_array();
        for chunk in arr.chunks(3) {
            if chunk[2].is_zero() {
                // division by zero not supported
                continue;
            }
            let a = builder.constant_nonnative::<BLS12381Scalar>(chunk[0]);
            let b = builder.constant_nonnative::<BLS12381Scalar>(chunk[1]);
            let d = builder.constant_nonnative::<BLS12381Scalar>(chunk[2]);

            let d_inv = builder.inv_nonnative(&d, Some(BLS12_381_SCALAR_LIMBS));
            let c_1 = builder.mul_nonnative(&a, &b);
            let c_1 = builder.mul_nonnative(&c_1, &d_inv); // a * b / d

            let c_2 = builder.mul_div_nonnative(&a, &b, &d); // a * b / d

            builder.connect_nonnative(&c_1, &c_2);
        }

        let data = builder.build::<C>();
        let proof = data.prove(PartialWitness::new()).unwrap();
        data.verify(proof)
    }
}
