...
use crate::liquidation::{
    get_available_shares_to_burn, get_shares_asset_value, get_shares_usdc_value,
};
...
impl Verify for L2ForceBurnSharesTxTarget {
    fn verify(&mut self, builder: &mut Builder, tx_type: &TxTypeTargets, tx_state: &TxState) {
        ...
        builder.conditional_assert_not_eq(
            is_enabled,
            tx_state.accounts[LIQUIDITY_POOL_ACCOUNT_ID].master_account_index,
            tx_state.accounts[SHARE_OWNER_ACCOUNT_ID].account_index,
        );
        ...
        // Pool can't be in liquidation
        let is_pool_in_liquidation = tx_state.risk_infos[LIQUIDITY_POOL_ACCOUNT_ID]
            .cross_risk_parameters
            .is_in_liquidation(builder);
        builder.conditional_assert_false(is_enabled, is_pool_in_liquidation);

        self.account_shares = tx_state.public_pool_shares[SHARE_OWNER_ACCOUNT_ID].share_amount;
        ...
        let available_shares_to_burn = get_available_shares_to_burn(
            builder,
            &tx_state.risk_infos[LIQUIDITY_POOL_ACCOUNT_ID].cross_risk_parameters,
...

        {
            ...
            let big_entry_usdc = builder.target_to_biguint(
                tx_state.public_pool_shares[SHARE_OWNER_ACCOUNT_ID].principal_amount,
            );
...

            {
                ...
                let big_tav = tx_state.risk_infos[LIQUIDITY_POOL_ACCOUNT_ID]
                    .cross_risk_parameters
                    .total_account_value
                    .abs
        ...
        let max_allowed_principal = {
            let staked_share_info = tx_state.accounts[SHARE_OWNER_ACCOUNT_ID]
...
impl Apply for L2ForceBurnSharesTxTarget {
    fn apply(&mut self, builder: &mut Builder, tx_state: &mut TxState) -> BoolTarget {
        ...
        tx_state.public_pool_shares[SHARE_OWNER_ACCOUNT_ID].principal_amount = builder.select(
            self.success,
        ...
        let new_share_amount = builder.sub(
            tx_state.public_pool_shares[SHARE_OWNER_ACCOUNT_ID].share_amount,
            total_burned_shares,
        );
        tx_state.public_pool_shares[SHARE_OWNER_ACCOUNT_ID].share_amount = builder.select(
            self.success,
            new_share_amount,
            tx_state.public_pool_shares[SHARE_OWNER_ACCOUNT_ID].share_amount,
        );
        ...
        tx_state.apply_pool_share_delta_flag =
            builder.select(self.success, two, tx_state.apply_pool_share_delta_flag);
