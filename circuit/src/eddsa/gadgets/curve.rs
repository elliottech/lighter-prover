...
pub trait CircuitBuilderEcGFp5 {
    ...
    fn conditional_ecgfp5_point_decode(
        &mut self,
        flag: BoolTarget,
        w: QuinticExtensionTarget,
    ) -> ECgFp5PointTarget;
...
impl CircuitBuilderEcGFp5 for Builder<F, D> {
    ...
    fn ecgfp5_add(&mut self, a: ECgFp5PointTarget, b: ECgFp5PointTarget) -> ECgFp5PointTarget {
        ...
        lambda_0_if_x_same = self.add_const_quintic_ext(lambda_0_if_x_same, WeierstrassPoint::A);
    ...
    fn precompute_window(
        ...
        for _ in 3..(1 << window_bits) {
            multiples.push(self.ecgfp5_add(*multiples.last().unwrap(), a));
        }
    ...
    fn conditional_ecgfp5_point_decode(
        ...
        let w_is_zero = self.is_equal_quintic_ext(w, zero_quintic_ext);
        let delta_is_sqrt_or_w_is_zero = self.or(delta_is_sqrt, w_is_zero);
        self.conditional_assert_true(flag, delta_is_sqrt_or_w_is_zero);
